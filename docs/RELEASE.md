...
これで以下が自動的に行われます:
1. 最新の成功ビルドから `Roster.exe` をダウンロード
2. あなたのデータフォルダ（`settings.json` の `data_dir`）から DB と写真を取得
3. `.lock` は除外（受け取った人で起動可能にするため）
4. `dist/Roster_dist_YYYYMMDD_HHMM.zip` を生成
...
2. `Roster.exe` をダブルクリック
3. 初回起動でデータフォルダ選択 → 同梱の `data` フォルダを指定
4. 296名のデータが読み込まれて利用開始
